...
import { Share2, ChevronRight, Crown, TrendingUp, Layers } from 'lucide-react';
import { type Entity, type AddressCluster } from '@/lib/schemas';
import { type NetworkKingpin, type RankedEntity } from '@/lib/leaderboard_rankings';
...
interface LeaderboardEntryProps {
    rank: number;
    data: RankedEntity | NetworkKingpin | AddressCluster | Entity;
...
export default function LeaderboardEntry({ rank, data, category, onShare, onDetails }: LeaderboardEntryProps) {
    ...
    if (category === 'risk' || category === 'phoenix') {
        const entity = data as RankedEntity;
        title = entity.name;
        if (entity.is_group) {
        ...
        const cluster = data as AddressCluster; // AddressCluster
            ...
            <div className={`flex items-center gap-4 ${isTop3 ? 'pl-4' : 'pl-2'}`}>
                ...
                <div className="flex-1 min-w-0">
                    <h3 className={`font-bold truncate group-hover:text-white transition-colors ${rank === 1 ? 'text-xl text-white' : 'text-zinc-200'}`}>
                        {title}
                        {('is_group' in data && data.is_group) && <Layers className="inline-block ml-2 w-4 h-4 text-zinc-500" />}
                    </h3>
