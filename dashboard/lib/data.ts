...
export function calculateRiskScore(entity: MasterlistEntity): number {
  ...
  let statusWeight = 0;
  if (statusUpper.includes('REVOKED')) statusWeight = 100;
  else if (statusUpper.includes('DENIED')) statusWeight = 90;
  else if (statusUpper.includes('SUSPENDED')) statusWeight = 80;
  else if (statusUpper.includes('CONDITIONAL')) statusWeight = 40;
  else if (statusUpper.includes('CLOSED')) statusWeight = 20;
  score += statusWeight;
