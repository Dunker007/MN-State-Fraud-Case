...
import { z } from 'zod';
import { Clock, Skull, Target, Network, FileWarning, Activity } from 'lucide-react';
...
export const PatternSchema = z.object({
    ...
    icon: z.any(), // store icon component directly or name? The usage below uses components.
    deepDive: z.object({
...
export const validatedPatterns = DETECTED_PATTERNS.map(p => PatternSchema.parse({
    ...p,
    id: String(p.id) // Ensure ID is string for generic schema
}));
